...
plot.fitCyclic <- function(x, plotType = c("raw","means","oneCycle")) {
  ...
  if (is.null(x$input$cov) & ("oneCycle" %in% plotType)) plot( x$oneCyclePlot) 
  if (!is.null(x$input$cov) & ("oneCycle" %in% plotType)) print("The oneCycle figure is not plotted because there are covariates") 
